...
def operation(path,filename,choice):
...
for f in files:
    try :
        ...
        im.close()
        time = [creation_time[i:i+10] for i in range(0, len(creation_time), 10)]
        t = str(time[0])
        needTime = t.replace(" ", "0")
        if creation_time is not None and needTime != "0000:00:00" and needTime != '':
            ...
            if checkDirectoryStatus== True :
                im.close()
                operation(path,filename,choice)
            else:
            ...
            basefilenameList=filename.split('.')
            basefilename=basefilenameList[0]
            phoneRegex = re.compile(r'(20\d{6})')
